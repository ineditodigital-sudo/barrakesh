    ...
    return (
        <div className="bg-background-dark font-display min-h-screen flex flex-col overflow-hidden relative selection:bg-primary selection:text-black">
            ...
            <header className="flex-none px-6 pt-8 pb-4 z-10 relative">
                ...
                <p className="text-neutral-400 text-xs font-mono uppercase mt-2 tracking-widest">/// Choose your barber</p>
            </header>

            {/* Main Carousel Area */}
            <main className="flex-1 w-full overflow-x-auto snap-x snap-mandatory flex gap-6 px-6 items-center no-scrollbar pb-12 relative z-0">
                {barbers.map((b) => {
                    const isAway = b.status === "AWAY";
                    return (
                        <div
                            key={b.id}
                            className={`snap-center shrink-0 w-[80vw] h-[55vh] relative group cursor-pointer ${isAway ? 'opacity-70 grayscale' : ''}`}
                        >
                            {/* Card Container */}
                            <div className="absolute inset-0 bg-surface rounded-sm overflow-hidden border border-neutral-800">
                                ...
                                {/* Vertical Name */}
                                <div className="absolute top-0 left-0 h-full w-20 flex items-center justify-center pointer-events-none z-10">
                                    <h2 className={`writing-vertical text-8xl font-bold uppercase tracking-tighter drop-shadow-xl whitespace-nowrap ${isAway ? 'text-outline-white opacity-50' : 'text-outline'}`}>
                                        {b.name}
                                    </h2>
                                </div>
                                ...
                                {!isAway && (
                                    <div className="absolute bottom-12 right-4 text-right">
                                        <p className="text-neutral-400 text-[10px] font-mono uppercase mb-1">Spec</p>
                                        {b.spec.map((s, idx) => (
                            ...
                            >
                                <span>{isAway ? 'Unavailable' : 'Select'}</span>
            ...
            <div className="flex-none bg-surface border-t border-neutral-800 px-6 py-6 font-mono uppercase">
                <div className="flex justify-between items-center text-xs">
                    <div className="text-neutral-500">
                        <span className="text-primary mr-1">///</span> Availability
                    </div>
                    <div className="flex gap-4">
                        <div className="text-white">Today <span className="text-primary">4 slots</span></div>
                        <div className="text-neutral-500">TMRW <span className="text-white">Full</span></div>
                    </div>
                </div>
            </div>
        </div>
